...
def add_intercept(df, name, exp, geno):
    """Calculate mean expression among individuals homozygous for reference allele."""
...
def combine_geno_files(file_list, hits):
    """Merge genotypes for all significant SNPs with the gene name and SNP rsID."""
    hits = hits[["gene", "ID"]]
    geno = pd.DataFrame()
    for f in file_list:
        ...
        tmp = pd.merge(tmp, curr_hits, left_index=True, 
...
def map_local_ancestry(geno_df, tracts=None):
    """Returns a DataFrame with one column for every individual and one row for
       every SNP. Each entry corresponds to the comma-delimited local ancestry 
       haplotypes for the individual at the given SNP."""
    ...
    if tracts is None:
        ...
        lanc_df = geno_df.apply(lambda x:
            african_mapper(x, tracts[tracts.nwd_id == x.name]), axis=0, result_type='expand')
...
def sum_local_ancestry(lanc_df):
    """Returns a DataFrame with one column for every individual and one row for 
       every SNP. Each entry corresponds to the number of African ancestry 
       tracts for the individual at the given SNP, summing across both haplotypes."""
...
def ancestry_phase_genotypes(geno_df, swap, lanc_df):
    """Returns a DataFrame with one column for every individual and one row for 
       every SNP. Each entry corresponds to the ancestry-phased genotype (i.e. 
       the number of alternative alleles on a European background and the 
       number of alternative alleles on an African background, separated by a 
       hyphen) for the individual at the given SNP."""
...
def prepare_merged_df(afr_hit_path, eur_hit_path, tract_path, n_genes, swap_alleles, cov_path):
    """Parses local ancestry tracts to determine ancestry-phased genotypes 
       (i.e. # of alt. alleles in European ancestry region vs # of alt. alleles 
       in African ancestry region). Resulting DataFrame has one row for each 
       SNP for each individual (i.e. total number of rows is nm, where n is 
       number of individuals and m is number of SNPs)."""
    # Merge significant SNPs from regions of African ancestry in AA (also 
    # called Afr-hom) and regions of European ancestry in EA
    ...
    # Read in local ancestry tracts (format: one row for each tract in each 
    # individual) and split tract info ID into separate columns
    tracts = pd.read_csv(tract_path, names=["chrom", "start", "stop", "ID"], sep='\t')
    tracts[["nwd_id", "haplotype", "ancestry"]] = tracts.apply(lambda x: 
    ...
    # Reads in genotype files and creates m x n matrix of ancestry-phased 
    # genotypes (i.e. one row for each SNP and one column for each individual)
    ...
    # Reads in phenotype files and creates m x n matrix of phenotypes 
    # (i.e. one row for each phenotype and one column for each individual)
    ...
    # Adds population-specific intercept by calculating mean expression in 
    # reference homozygotes in each pop. This is occasionally used as an 
    # initialization for population-specific intercepts to fit the delta in
    # iterative_parameter_optimization.py
    hits = add_intercept(hits, "Afr", afr_expression, afr_genotypes)
    hits = add_intercept(hits, "Eur", eur_expression, eur_genotypes)

    # Reshapes genotype and phenotype dataframes such that there is one row for 
    # each SNP/phenotype for each individual
    ...
    merged_data[["genotype_Eur", "genotype_Afr"]] = merged_data.apply(lambda x: 
        pd.Series([int(x.genotype[0]), int(x.genotype[2])]), axis=1)
