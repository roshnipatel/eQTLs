...
parser.add_argument("--global_anc", help="file with global ancestry info")
parser.add_argument("--pca", help="file with expression PCs", nargs='+')
...
ganc = ganc[["ID", "YRI_admixture"]].rename({"ID": "nwd_id", "YRI_admixture": "global_ancestry"}, axis=1)
samples = pd.merge(samples, sample_metadata[["tor_id", "nwd_id", "seq_center", "exam", "age", "sex", "race"]])
