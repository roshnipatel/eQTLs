...
tracts_df["NWDID"] = tracts_df["Info"].apply(lambda x: x.split('_')[0], axis=1)
...
for _, SNP in SNP_list.iteritems(): 
    ...
    end_SNP_window = min(pos + 200000, chrom_lengths[chrom_lengths["Chrom"] == chrom]["Length"].iloc[0])
    ...
    end_region = min(pos + region_size, chrom_lengths[chrom_lengths["Chrom"] == chrom]["Length"].iloc[0])
